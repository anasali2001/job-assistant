...
# UPDATE MEMORY
# ==========================================
def update_memory(doc_id, new_text):

    meta = extract_metadata(new_text)
    ...
    vector = embedder.encode(new_text).tolist()
...
def analyze_intent(query, project_filter=None):
    ...
    if project_filter and project_filter != "All":
        filter_dict = {"project": {"$eq": project_filter}}
    ...
    if not matches:
        return {"type":"message","content":"No memory found"}
...
1 DELETE memory
Return:
CONFIRM_DELETE: <ID> | <Summary>

2 EDIT memory
Return:
CONFIRM_EDIT: <ID> | <Summary> | <NEW TEXT>
...
with st.sidebar:
    ...
    try:
        ...
        if not df_all.empty:
            projects=["All"]+list(df_all.project.unique())
        else:
            projects=["All"]
    ...
    selected_project=st.selectbox("Project Filter",projects)

# ==========================================
# TABS
# ==========================================
tab1,tab2,tab3=st.tabs(["Chat","Analytics","Recent Logs"])
...
with tab1:
    ...
    if st.session_state.pending_edit:
        ...
        st.warning("Edit memory")

        edited=st.text_area(
            "Modify text",
            value=st.session_state.pending_edit["new_text"]
        )
        ...
        if c1.button("Update"):
            ...
            update_memory(
                st.session_state.pending_edit["id"],
                edited
            )
    ...
    # CHAT INPUT
    prompt=st.chat_input("Ask or /log")
    ...
    if prompt:
        ...
        if prompt.startswith("/log"):
            ...
            elif res["type"]=="confirm_edit":

                st.session_state.pending_edit=res
...
with tab2:
    ...
    if not df_all.empty:
        ...
        fig=px.pie(df_all,names="project")
