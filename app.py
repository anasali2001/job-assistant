...
def extract_metadata(text):
    system_prompt = "Extract metadata. Return ONLY valid JSON. Fields: 'project' (default 'General'), 'tags' (list of strings)."
    try:
        response = llm_client.chat.completions.create(
            model=LLM_MODEL,
            messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": text}],
...
def analyze_intent(query, project_filter=None):
    ...
    matches = results['matches']
    if not matches: return {"type": "message", "content": "📭 No memories found."}
    
    context_str = "\n".join([f"ID: {m['id']} | Date: {m['metadata'].get('date')} | Text: {m['metadata'].get('text')}" for m in matches])
    
    system_prompt = "You are a Second Brain. Rules: If user wants to DELETE a log, return: `CONFIRM_DELETE: <ID> | <Summary>`. Else, answer helpfully."
    response = llm_client.chat.completions.create(
        model=LLM_MODEL,
    ...
    if content.startswith("CONFIRM_DELETE:"):
        try:
            parts = content.split(":", 1)[1].split("|")
            return {"type": "confirm_delete", "id": parts[0].strip(), "summary": parts[1].strip()}
        except: return {"type": "message", "content": "❌ Error parsing deletion."}
    return {"type": "message", "content": content}
...
with tab3:
    if not df_all.empty: st.dataframe(df_all[['date', 'project', 'text']], use_container_width=True)
