...
# ==========================================
st.set_page_config(
    page_title="CRIPT Vision Assistant", 
    page_icon="👁️", 
    layout="wide",
...
def check_password():
    env_pass = os.getenv("APP_PASSWORD")
    if env_pass and st.session_state.password_input.strip() == env_pass.strip():
        st.session_state.authenticated = True
    else:
        st.error("⛔ Invalid Credentials")

if not st.session_state.authenticated:
    st.markdown("<h1 style='text-align: center;'>👁️ CRIPT Agency: CV Engineer Portal</h1>", unsafe_allow_html=True)
...
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY")
PINECONE_API_KEY = os.getenv("PINECONE_API_KEY")
INDEX_NAME = "ubl-brain" # Kept original name as requested
...
def init_services():
    llm = OpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=OPENROUTER_API_KEY,
        default_headers={"X-Title": "CRIPT Vision Assistant"}
    )
    embedder = SentenceTransformer('all-MiniLM-L6-v2')
    pc = Pinecone(api_key=PINECONE_API_KEY)
    index = pc.Index(INDEX_NAME)
...
# ==========================================
def extract_metadata(text):
    system_prompt = """You are a technical assistant for a Computer Vision Engineer. 
    Analyze the text. Categories: 'Computer Vision', 'MLOps', 'GenAI', 'Architecture'.
    Return ONLY JSON: {"project": "CategoryName", "tags": ["tag1", "tag2"]}."""
    try:
        response = llm_client.chat.completions.create(
        ...
        return {"project": "General", "tags": "Misc"}
...
def save_memory(text):
    ...
    meta["text"] = text
    if isinstance(meta.get("tags"), list): meta["tags"] = ", ".join(meta["tags"])
    index.upsert(vectors=[(str(uuid.uuid4()), embedder.encode(text).tolist(), meta)])
    return meta

def analyze_intent(query, project_filter):
    results = index.query(vector=embedder.encode(query).tolist(), top_k=5, include_metadata=True, 
                          filter={"project": {"$eq": project_filter}} if project_filter != "All" else None)
    
    context = "\n".join([f"Data: {m['metadata']['text']}" for m in results['matches']])
    system_prompt = """You are a Computer Vision Engineer's expert assistant at CRIPT Agency.
    Expertise: OpenCV, PyTorch, RAG, MLOps, and production-grade AI pipelines.
    Provide actionable, highly technical advice. If user wants to delete, return: CONFIRM_DELETE: <ID> | <Summary>."""
...
# ==========================================
with st.sidebar:
    st.title("👁️ CRIPT Agency")
    if st.button("Logout"):
        st.session_state.authenticated = False
        st.rerun()

tab1, tab2 = st.tabs(["💬 Chat", "🗂️ Data Logs"])

with tab1:
    if "messages" not in st.session_state:
        st.session_state.messages = [{"role": "assistant", "content": "Ready for vision pipelines and model optimization."}]
    
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]): st.markdown(msg["content"])

    if prompt := st.chat_input("Enter log or question..."):
        st.session_state.messages.append({"role": "user", "content": prompt})
        st.chat_message("user").markdown(prompt)
        
        if prompt.lower().startswith("/log "):
            meta = save_memory(prompt[5:])
            res = f"✅ Saved to {meta['project']} pipeline."
        else:
            res = analyze_intent(prompt, "All")
            
        st.session_state.messages.append({"role": "assistant", "content": res})
        st.rerun()

with tab2:
    stats = index.query(vector=[0.1]*384, top_k=100, include_metadata=True)
    df = pd.DataFrame([m['metadata'] for m in stats['matches']]) if stats['matches'] else pd.DataFrame()
